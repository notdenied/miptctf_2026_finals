...
Флагов кладётся больше, чем получится украсть интендед уязвимостями. (upd: нет, я передумал, к счастью, не во время CTF)
...
Простой IDOR, через который можно вмержить чужой счет в свой и получить его имя. Очевидно ищется ручками, но плохо ищется по коду (и не ищется иишкой? по крайней мере у меня не нашлось с 1 раза, уже неплохо) -> хороший кандидат на уязу. Плюс айди инкрементальные, а значит -- можно не давать аттак дату.

3. http requests smuggling через lighttpd (CVE-2025-12642).

(да, я плохой человек и дал CVE)
...
Можем искать по любому полю -> можем искать по id + ключу для доступа к приватному (протектед) посту...
...
Увынск, но я почему-то клал флаги в приватные посты, а не протектед, пупупу...
...
Увы, но это привело к чуть большему дудосу, чем я бы хотел, но вроде все устояли).
